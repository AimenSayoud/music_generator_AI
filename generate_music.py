...
class MusicGenerator:
    def __init__(self, fs=32, min_active_notes=1, max_active_notes=8, note_repeat_threshold=4):
        # Basic parameters
        self.fs = fs
        self.min_active_notes = min_active_notes
        self.max_active_notes = max_active_notes
        self.note_repeat_threshold = note_repeat_threshold
        ...
        self.last_notes = []
        # self.note_repeat_threshold = 4  # Prevent excessive repetition
...
def create_midi_file(notes, velocities, output_file='generated_music.mid', fs=32, tempo=100, velocity_range=(50, 127)):
    """Create MIDI file with improved dynamics and velocity range control"""
    ...
    velocity_range_size = velocity_max - velocity_min
    ...
    for step in range(notes.shape[0]):
        ...
        for note_num in range(notes.shape[1]):
            ...
            if is_active and note_states[note_num] == 0:
                ...
                # Scale velocity to the target range
                normalized_velocity = current_velocity  # Already in [0,1]
                
                # Calculate base velocity in the target range
                base_velocity = int(velocity_min + (normalized_velocity * velocity_range_size))
                
                # Apply dynamic adjustments while respecting velocity range
                if bar_position == 0:  # Bar start
                    velocity = min(velocity_max, int(base_velocity * 1.4))
                elif beat_position == 0:  # Beat start
                    velocity = min(velocity_max, int(base_velocity * 1.3))
                elif beat_position % (fs // 2) == 0:  # Eighth notes
                    velocity = min(velocity_max, int(base_velocity * 1.2))
                else:
    ...
    print(f"Velocity range used: {velocity_min} to {velocity_max}")
...
def parse_arguments():
    ...
    parser.add_argument('--length', type=int, default=1024,
                       help='Length of generated sequence in steps')
    parser.add_argument('--min-notes', type=int, default=1,
                       help='Minimum active notes at once')
    parser.add_argument('--max-notes', type=int, default=8,
                       help='Maximum active notes at once')
    
    # Musical parameters
    parser.add_argument('--tempo', type=int, default=100,
                       help='Tempo in BPM')
    parser.add_argument('--velocity-min', type=int, default=50,
                       help='Minimum note velocity')
    parser.add_argument('--velocity-max', type=int, default=127,
                       help='Maximum note velocity')
    
    # Model parameters
    parser.add_argument('--model-path', type=str, default='music_generator_model.pth',
                       help='Path to trained model')
    parser.add_argument('--data-path', type=str, default='processed_music_data.pkl',
                       help='Path to processed data')
    parser.add_argument('--output-file', type=str, default='generated_music.mid',
                       help='Output MIDI file path')
    
    # Generation control
    parser.add_argument('--seed-tries', type=int, default=5,
                       help='Number of attempts to find good seed')
    parser.add_argument('--note-repeat-threshold', type=int, default=4,
                       help='Note repetition control threshold')
...
def main():
    ...
    try:
        ...
        for _ in range(args.seed_tries):
        ...
        # Print seed info
        active_notes_in_seed = np.sum(seed_seq.sum(axis=1) > 0)
        print(f"Seed sequence stats:")
        print(f"- Length: {len(seed_seq)} steps")
        print(f"- Active timesteps: {active_notes_in_seed}")
        print(f"- Note density: {seed_seq.sum() / len(seed_seq):.1f} notes per step")
        
        # Prepare tensors
        seed_sequence = torch.FloatTensor(seed_seq[:128]).unsqueeze(0).to(device)
        seed_velocities = torch.FloatTensor(seed_vel[:128]).unsqueeze(0).to(device)
        
        # Initialize generator with parameters
        generator = MusicGenerator(
            fs=args.fs,
            min_active_notes=args.min_notes,
            max_active_notes=args.max_notes,
            note_repeat_threshold=args.note_repeat_threshold
        )
        ...
        # Create MIDI with parameters
        print(f"\nCreating MIDI file: {args.output_file}")
        success = create_midi_file(
            generated_notes,
